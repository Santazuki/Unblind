...
- 输出测试报告到 `docs/test-reports/`

## 工作流程
1. 获取即将测试的代码变更（通过 `git diff` 或开发者说明）
2. 编写/补充测试文件（使用 Jest 或项目指定框架）
3. 运行 `npm test`，收集结果
...
当任务没有依赖关系时，主对话可以**同时触发多个 Subagent**。例如：

- 同时调用 `@tester` 运行回归测试，`@reviewer` 审查代码变更
- 或者同时让两个不同的 `@developer` 实例实现两个相互独立的模块

Claude Code 原生支持并发工具调用，主对话只需一次性列出多个 Subagent 调用即可。
